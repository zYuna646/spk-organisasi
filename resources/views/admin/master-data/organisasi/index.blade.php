...
@section('content')
    <div class="card bg-light-info shadow-none position-relative overflow-hidden">
        <div class="card-body px-4 py-3">
            <h4 class="fw-semibold mb-8">{{ $title ?? 'Organisasi' }}</h4>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item">
                        <a href="{{ route('admin.dashboard') }}" class="text-muted">Dashboard</a>
                    </li>
                    <li class="breadcrumb-item active" aria-current="page">{{ $title ?? 'Organisasi' }}</li>
                </ol>
            </nav>
        </div>
    </div>
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-8 col-xl-9 text-end d-flex justify-content-md-end justify-content-center mt-3 mt-md-0">
                    <a href="{{ route('admin.organisasi.create') }}" class="btn btn-info d-flex align-items-center">
                        <i class="ti ti-plus text-white me-1 fs-5"></i> Add Organization
                    </a>
                </div>
            </div>
        </div>
                    ...
                    <tbody>
                        @foreach ($datas as $result)
                            <tr>
                                ...
                                <td>{{ $result->ketua_umum }}</td>
                                <td>{{ $result->user->email }}</td>
                                <td>{{ $result->tahun_berdiri }}</td>
                                <td>
                                    <a href="{{ route('admin.organisasi.show', $result->id) }}"
                                        class="btn btn-sm btn-secondary">
                                        <i class="ti ti-eye"></i>
                                    </a>
                                    <a href="{{ route('admin.organisasi.edit', $result->id) }}"
                                        class="btn btn-sm btn-warning">
                                        <i class="ti ti-pencil"></i>
                                    </a>
                                    <form action="{{ route('admin.organisasi.destroy', $result->id) }}"
        ...
        <div class="card-body">
            <div class="alert alert-warning mb-0" role="alert">
                <div class="d-flex gap-2 align-items-center">
                    ...
                    <p class="mb-0">
                        No organizations available. <a href="{{ route('admin.organisasi.create') }}">Add</a> now.
                    </p>
                </div>
            </div>
        </div>
